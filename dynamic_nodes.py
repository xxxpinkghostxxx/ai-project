...
def add_dynamic_nodes(graph, num_dynamic=None):
    ...
    """
    num_sensory = len([lbl for lbl in graph.node_labels if lbl.get('type', 'sensory') == 'sensory' or 'channel' in lbl])
    if num_dynamic is None:
        num_dynamic = 4 * num_sensory
    # Random energy values for dynamic nodes
    dynamic_energies = np.random.uniform(0, MAX_DYNAMIC_ENERGY, size=(num_dynamic, 1)).astype(np.float32)
    ...
    for i in range(num_dynamic):
        ...
        graph.node_labels.append({'id': node_id, 'type': 'dynamic', 'energy': energy})
    return graph
