...
_energy_cap_cache_ttl = 60
_enhanced_nodes_cache = None
_enhanced_nodes_cache_time = 0
_enhanced_nodes_cache_ttl = 60
...
def get_enhanced_nodes_config_cached() -> Dict[str, float]:
    global _enhanced_nodes_cache, _enhanced_nodes_cache_time
    import time
    current_time = time.time()
    if (_enhanced_nodes_cache is not None and
        current_time - _enhanced_nodes_cache_time < _enhanced_nodes_cache_ttl):
        return _enhanced_nodes_cache
    _enhanced_nodes_cache = get_enhanced_nodes_config()
    _enhanced_nodes_cache_time = current_time
    return _enhanced_nodes_cache
...
class BehaviorEngine:
    ...
    def update_node_behavior(self, node_id: int, graph: Data, step: int, access_layer=None) -> bool:
        ...
        if has_enhanced_behavior and self.enhanced_integration is not None:
            try:
                if not hasattr(self.enhanced_integration, 'node_behavior_system') or self.enhanced_integration.node_behavior_system is None:
                    ...
                    if enhanced_behavior is not None:
        ...
        try:
            ...
            if self.error_handler is not None:
                try:
                    recovery_success = self.error_handler.handle_error(
                        e, f"Error updating {behavior} node",
            ...
            if self.error_handler is not None:
                try:
                    recovery_success = self.error_handler.handle_error(
                        e, f"Unexpected error updating {behavior} node",
                        recovery_func=lambda: self._recover_node_behavior(node_id, access_layer),
                        critical=False
                    )
                    if recovery_success:
                        log_step(f"Recovered from unexpected {behavior} node error", node_id=node_id)
                        access_layer.update_node_property(node_id, 'last_update', step)
                        return True
                except Exception as recovery_error:
                    log_step(f"Recovery failed for unexpected {behavior} node error", error=str(recovery_error), node_id=node_id)
            try:
                access_layer.update_node_property(node_id, 'last_update', step)
            except (AttributeError, KeyError, TypeError):
                pass
