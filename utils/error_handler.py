...
"""
Basic Error Handler - Simplified interface to the unified error handling system.
This module provides a simple interface while delegating to the comprehensive
unified_error_handler.py for advanced functionality.
"""
...
# Import the unified error handler for advanced functionality
try:
    from utils.unified_error_handler import UnifiedErrorHandler, ErrorSeverity, ErrorCategory
    _unified_handler = UnifiedErrorHandler()
except ImportError:
    _unified_handler = None
    log_step("Warning: unified_error_handler not available, using basic error handling")
...
class ErrorHandler:
    ...
    def handle_error(self, error: Exception, context: str = "",
        ...
        """Handle error with delegation to unified handler if available."""
        # Delegate to unified handler if available
        if _unified_handler is not None:
            try:
                severity = ErrorSeverity.CRITICAL if critical else ErrorSeverity.MEDIUM
                category = ErrorCategory.SYSTEM
                return _unified_handler.handle_error(
                    error=error,
                    context=context,
                    severity=severity,
                    category=category,
                ...
                log_step("Error in unified handler delegation", error=str(e))
