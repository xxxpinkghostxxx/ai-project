...
class PyGNeuralSystem:
    ...
    def cleanup(self) -> None:
        ...
        logger.info("Cleanup called. Setting self.g to None.")
        try:
    ...
    def _check_memory_usage(self) -> None:
        ...
        if current_time - self._memory_tracker['last_cleanup'] > self._memory_tracker['cleanup_interval']:
            ...
            if (self._memory_tracker['peak_nodes'] > self.n_total * 2 or
                (self.g is not None and self.g.num_edges is not None and self._memory_tracker['peak_edges'] > self.g.num_edges * 2)):
                self.cleanup()
    ...
    def _get_unoccupied_grid_positions(self, n: int, exclude_mask: Optional[torch.Tensor] = None, grid_width: Optional[int] = None, grid_height: Optional[int] = None, min_dist: float = 0.4, fast_unique_grid: bool = False) -> Tuple[torch.Tensor, bool]:
        ...
        if n == 1:
            ...
            selected_cells = [free_cells[0]]
            if n > 1:
                selected_arr = np.array(selected_cells)
        ...
        if len(selected_cells) < n:
            ...
            self.grid_width = width
            self.grid_height = height
            return torch.from_numpy(np.array(selected_cells)).float().to(self.device), False
        return torch.from_numpy(np.array(selected_cells)).float().to(self.device), True
    ...
    def _init_graph(self) -> None:
        ...
        if self.n_dynamic > 0 and num_dyn_dyn > 0:
            ...
            edge_conn_subtype3.append(conn_subtype3)
        # Build PyG graph
        src = torch.cat([e for e in edge_src if e is not None]) if edge_src else torch.tensor([], dtype=torch.int64, device=device)
        dst = torch.cat([e for e in edge_dst if e is not None]) if edge_dst else torch.tensor([], dtype=torch.int64, device=device)
        ...
        if edge_weights:
            self.g.weight = torch.cat([w for w in edge_weights if w is not None]).unsqueeze(1)
            self.g.energy_transfer_capacity = torch.cat([c for c in edge_caps if c is not None]).unsqueeze(1)
            self.g.conn_type = torch.cat([t for t in edge_types if t is not None]).unsqueeze(1)
            self.g.plastic_lr = torch.cat([l for l in edge_plastic_lrs if l is not None]).unsqueeze(1)
            self.g.gate_threshold = torch.cat([g for g in edge_gate_thresholds if g is not None]).unsqueeze(1)
            self.g.conn_subtype2 = torch.cat([s for s in edge_conn_subtype2 if s is not None])
            self.g.conn_subtype3 = torch.cat([s for s in edge_conn_subtype3 if s is not None])
    ...
    def update_sensory_nodes(self, sensory_input: Union[np.ndarray, torch.Tensor]) -> None:
        ...
        try:
            ...
            # Update energies
            self.g.energy[sensory_mask] = sensory_input.flatten().unsqueeze(1)
    ...
    def _update_energies(self) -> None:
        ...
        if g is None:
            return
        if g.num_nodes is None or g.num_edges is None:
            return
        ...
        if dynamic_mask.sum() > 0:
            ...
            if g.edge_index is not None:
                out_deg = torch.zeros(g.num_nodes, dtype=torch.float, device=self.device).scatter_add_(0, g.edge_index[0], torch.ones(g.num_edges, device=self.device))
                decay = out_deg[dynamic_mask] * CONNECTION_MAINTENANCE_COST
                g.energy[dynamic_mask] -= decay.unsqueeze(1)

                # Calculate energy gain from outgoing connections (vectorized)
                src, dst = g.edge_index
                weights = g.weight.squeeze()
                conn_subtype3 = g.conn_subtype3 if hasattr(g, 'conn_subtype3') else torch.zeros_like(weights, dtype=torch.int64)
                if g.num_nodes is None:
                    raise ValueError("g.num_nodes is None")
                parent = g.parent if hasattr(g, 'parent') else torch.full((int(g.num_nodes or 0),), -1, dtype=torch.int64, device=g.device)
                ...
                g.energy[:,0] += 0.1 * outgoing_energy
        ...
        for node_type, energy_field in [
            ...
            if mask.sum() > 0:
                g.energy[mask] = torch.zeros_like(g.energy[mask])
                # Reset energy for sensory and workspace nodes.
                # In PyTorch Geometric, message passing is handled through custom MessagePassing layers.
                if hasattr(g, 'energy_in'):
                    g.energy += g.energy_in
                    del g.energy_in

        # Apply energy caps and death threshold
        g.energy.clamp_(NODE_DEATH_THRESHOLD, NODE_ENERGY_CAP)

        # Kill nodes that are below death threshold
        dead_nodes = (g.energy <= NODE_DEATH_THRESHOLD).squeeze()
        if dead_nodes.any():
    ...
    def _guarantee_minimum_connections(self, node_indices: Optional[torch.Tensor] = None, batch_size: int = 500) -> bool:
        """
        ...
        """
        if self.g is None or self.g.num_nodes is None or self.g.num_edges is None:
            return False
        g = self.g
        device = self.device
        if node_indices is None:
            if g.num_nodes is not None:
                ...
                return False
        if g.edge_index is not None and g.num_nodes is not None and g.num_edges is not None:
            ...
            for i in range(0, len(no_conn), batch_size):
                ...
                for src in batch:
                    ...
                    ws_mask = (g.node_type[valid_targets] == NODE_TYPE_WORKSPACE)
                    dyn_mask = (g.node_type[valid_targets] == NODE_TYPE_DYNAMIC)
                    sens_mask = (g.node_type[valid_targets] == NODE_TYPE_SENSORY)
                    if ws_mask.any():
                        dst = valid_targets[ws_mask][int(torch.randint(0, int(ws_mask.sum()), (1,)).item())]
                    elif dyn_mask.any():
                        dst = valid_targets[dyn_mask][int(torch.randint(0, int(dyn_mask.sum()), (1,)).item())]
                    elif sens_mask.any():
                        dst = valid_targets[sens_mask][int(torch.randint(0, int(sens_mask.sum()), (1,)).item())]
                    else:
                        continue
                    src_list.append(src)
                    dst_list.append(dst)
            ...
            if g.edge_index is not None:
                out_deg = torch.zeros(g.num_nodes, dtype=torch.long, device=device).scatter_add_(0, g.edge_index[0], torch.ones(g.num_edges, dtype=torch.long, device=device))
                return (out_deg[node_indices] > 0).all().item()  # type: ignore[return-value]
            else:
                return False
        else:
            return False
    ...
    def _cull_invalid_connections(self) -> int:
        ...
        parent = g.parent if hasattr(g, 'parent') else torch.full((g.num_nodes,), -1, dtype=torch.int64, device=g.device)
        is_parent = (parent[dst] == src)
        # Valid if:
        # - FREE_FLOW
        ...
        if to_remove.any():
            ...
            g.edge_index = g.edge_index[:, ~to_remove]
            for key in g.keys():
                if hasattr(g, key) and g[key].dim() > 0 and g[key].shape[0] == original_num_edges:
                    g[key] = g[key][~to_remove]
        return to_remove.sum().item()
    ...
    def _prepare_connection_growth_batch(self, batch_size: int) -> List[Any]:
    ...
    def _prepare_cull_batch(self, batch_size: int) -> List[int]:
        # Prepare a batch of edge indices to remove (do not modify graph here)
        if self.g is None or self.g.num_nodes is None or self.g.num_edges is None or self.g.edge_index is None:
            return []
        g = self.g
        assert g.num_nodes is not None
        assert g.num_edges is not None
    ...
    def apply_connection_worker_results(self) -> None:
        ...
        if not hasattr(self, 'connection_worker') or self.g is None or self.g.num_nodes is None:
            return
            ...
            if result['type'] == 'grow':
                ...
                for src, dst, subtype3 in zip(src_list, dst_list, subtype3_list):
                    ...
                    if s_type == NODE_TYPE_DYNAMIC:
                        # Calculate out-degree for this node in PyG
                        if g.edge_index is not None:
                ...
                g = self.g
                if g.num_edges is None or g.edge_index is None:
                    continue
                ...
                if len(valid_eids) > 0:
                    ...
                    mask = torch.ones(g.num_edges, dtype=torch.bool, device=g.device)
                    mask[valid_eids] = False
                    g.edge_index = g.edge_index[:, mask]
                    for key in g.keys():
                        if hasattr(g, key) and g[key] is not None and g[key].dim() > 0 and g[key].shape[0] == g.num_edges:
                            g[key] = g[key][mask]
            self.connection_worker.result_queue.task_done()

    def _add_nodes(self, n: int, node_type: int, parent_idx: Optional[List[int]] = None) -> None:
        if self.g is None:
            return
        g = self.g
        device = self.device
    ...
    def _event_driven_transfer(self) -> None:
        ...
        transfer = src_energy * weights.squeeze() * energy_caps.squeeze()
