import sys
import torch
import torch_geometric
from torch_geometric.data import Data
from torch_geometric.utils import dense_to_sparse, to_dense_adj
import numpy as np
import time
import threading
import queue
import logging
from typing import Optional, Tuple, List, Dict, Any
from dataclasses import dataclass
from concurrent.futures import ThreadPoolExecutor

# Configure logging
logger = logging.getLogger(__name__)

# Try to import CUDA kernels, fallback gracefully if not available
try:
    from .cuda_kernels import CUDAModule
except ImportError:
    logger.warning("CUDA kernels not available, falling back to CPU implementation")
    CUDAModule = None

print("Starting PyG Neural System initialization...")
print(f"PyTorch version: {torch.__version__}")
print(f"PyTorch Geometric version: {torch_geometric.__version__}")
print(f"CUDA available: {torch.cuda.is_available()}")
if torch.cuda.is_available():
    print(f"CUDA device: {torch.cuda.get_device_name(0)}")

# --- Node Type Constants ---
NODE_TYPE_SENSORY = 0
NODE_TYPE_DYNAMIC = 1
NODE_TYPE_WORKSPACE = 2
NODE_TYPE_HIGHWAY = 3
NODE_TYPE_NAMES = ['Sensory', 'Dynamic', 'Workspace', 'Highway']

# --- Dynamic Node Subtypes ---
SUBTYPE_TRANSMITTER = 0
SUBTYPE_RESONATOR = 1
SUBTYPE_DAMPENER = 2
SUBTYPE_NAMES = ['Transmitter', 'Resonator', 'Dampener']

# --- Third Subtype Slot (Connection Call Frequency) ---
SUBTYPE3_1P_5 = 0  # 1% every 5 steps
SUBTYPE3_2P_10 = 1  # 2% every 10 steps
SUBTYPE3_3P_20 = 2  # 3% every 20 steps
SUBTYPE3_NAMES = ['1%/5steps', '2%/10steps', '3%/20steps']

# --- Fourth Subtype Slot (Connection-based Energy Gain) ---
SUBTYPE4_GAIN_1 = 0   # Gain 1 per connection per step
SUBTYPE4_GAIN_01 = 1  # Gain 0.1 per connection per step
SUBTYPE4_GAIN_001 = 2  # Gain 0.01 per connection per step
SUBTYPE4_NAMES = ['+1/conn', '+0.1/conn', '+0.01/conn']

# --- Config-like constants (tune as needed) ---
NODE_SPAWN_THRESHOLD = 20.0
NODE_DEATH_THRESHOLD = 0.0
NODE_ENERGY_SPAWN_COST = 5.0
NODE_ENERGY_DECAY = 0.1
MAX_NODE_BIRTHS_PER_STEP = 1000
MAX_CONN_BIRTHS_PER_STEP = 1000
NODE_ENERGY_CAP = 244.0
CONNECTION_MAINTENANCE_COST = 0.02
TRANSMISSION_LOSS = 0.9

# --- Connection Types ---
CONN_TYPE_EXCITATORY = 0
CONN_TYPE_INHIBITORY = 1
CONN_TYPE_GATED = 2
CONN_TYPE_PLASTIC = 3
CONN_TYPE_NAMES = ['Excitatory', 'Inhibitory', 'Gated', 'Plastic']
GATE_THRESHOLD = 0.5

# For plastic connections
PLASTIC_LEARNING_RATE_MIN = 0.001
PLASTIC_LEARNING_RATE_MAX = 0.05

# --- New Connection Subtype3 (Directionality) ---
CONN_SUBTYPE3_ONE_WAY_OUT = 0
CONN_SUBTYPE3_ONE_WAY_IN = 1
CONN_SUBTYPE3_FREE_FLOW = 2
CONN_SUBTYPE3_NAMES = ['OneWayOut', 'OneWayIn', 'FreeFlow']

@dataclass
class NodeData:
    """Data class for node attributes"""
    energy: torch.Tensor
    node_type: torch.Tensor
    pos: torch.Tensor
    velocity: torch.Tensor
    dynamic_subtype: Optional[torch.Tensor] = None
    dynamic_subtype2: Optional[torch.Tensor] = None
    dynamic_subtype3: Optional[torch.Tensor] = None
    dynamic_subtype4: Optional[torch.Tensor] = None
    max_connections: Optional[torch.Tensor] = None
    phase_offset: Optional[torch.Tensor] = None
    parent: Optional[torch.Tensor] = None

@dataclass
class EdgeData:
    """Data class for edge attributes"""
    weight: torch.Tensor
    energy_transfer_capacity: torch.Tensor
    conn_type: torch.Tensor
    plastic_lr: torch.Tensor
    gate_threshold: torch.Tensor
    conn_subtype3: torch.Tensor

class ConnectionWorker(threading.Thread):
    """Worker thread for handling connection operations in parallel"""
    def __init__(self, system, batch_size=25):
        super().__init__()
        self.system = system
        self.batch_size = batch_size
        self.task_queue = queue.Queue()
        self.result_queue = queue.Queue()
        self.stop_event = threading.Event()
        self.daemon = True
        self._lock = threading.Lock()
        self._processing = False
        self._error_count = 0
        self._max_retries = 3
        self._retry_delay = 1.0
        self._last_activity = time.time()
        self._timeout = 30.0  # 30 seconds timeout
        self._metrics = {
            'tasks_processed': 0,
            'errors': 0,
            'retries': 0,
            'processing_time': 0.0
        }

    def run(self):
        """Main worker loop with error handling and recovery"""
        while not self.stop_event.is_set():
            try:
                # Check for timeout
                if time.time() - self._last_activity > self._timeout:
                    logger.warning("Connection worker timeout, restarting...")
                    self._error_count += 1
                    if self._error_count >= self._max_retries:
                        logger.error("Max retries exceeded, stopping worker")
                        self.stop_event.set()
                        break
                    time.sleep(self._retry_delay * (2 ** (self._error_count - 1)))
                    continue

                # Get task with timeout
                try:
                    task = self.task_queue.get(timeout=0.1)
                except queue.Empty:
                    continue

                with self._lock:
                    self._processing = True
                    start_time = time.time()
                    try:
                        if task['type'] == 'grow':
                            # Prepare connection growth batch
                            batch = self.system._prepare_connection_growth_batch(self.batch_size)
                            self.result_queue.put({'type': 'grow', 'batch': batch})
                        elif task['type'] == 'cull':
                            # Prepare culling batch
                            batch = self.system._prepare_cull_batch(self.batch_size)
                            self.result_queue.put({'type': 'cull', 'batch': batch})
                        else:
                            logger.error(f"Unknown task type: {task['type']}")
                            self.result_queue.put({'type': 'error', 'error': f"Unknown task type: {task['type']}"})

                        # Update metrics
                        self._metrics['tasks_processed'] += 1
                        self._metrics['processing_time'] += time.time() - start_time
                        self._last_activity = time.time()
                        self._error_count = 0  # Reset error count on success

                    except Exception as e:
                        self._metrics['errors'] += 1
                        logger.error(f"Error processing task: {e}")
                        self.result_queue.put({'type': 'error', 'error': str(e)})
                        self._error_count += 1
                        if self._error_count >= self._max_retries:
                            logger.error("Max retries exceeded, stopping worker")
                            self.stop_event.set()
                            break
                    finally:
                        self._processing = False
                        self.task_queue.task_done()

            except Exception as e:
                logger.error(f"Critical error in connection worker: {e}")
                self._metrics['errors'] += 1
                self._error_count += 1
                if self._error_count >= self._max_retries:
                    logger.error("Max retries exceeded, stopping worker")
                    self.stop_event.set()
                    break
                time.sleep(self._retry_delay * (2 ** (self._error_count - 1)))

    def stop(self):
        """Safely stop the worker thread"""
        with self._lock:
            if self._processing:
                # Wait for current processing to complete
                while self._processing:
                    time.sleep(0.1)
            self.stop_event.set()
            logger.info("Connection worker stopped")

    def is_processing(self):
        """Check if worker is currently processing a task"""
        with self._lock:
            return self._processing

    def clear_queues(self):
        """Clear all pending tasks and results"""
        with self._lock:
            while not self.task_queue.empty():
                try:
                    self.task_queue.get_nowait()
                    self.task_queue.task_done()
                except queue.Empty:
                    break
            while not self.result_queue.empty():
                try:
                    self.result_queue.get_nowait()
                except queue.Empty:
                    break
            logger.info("Connection worker queues cleared")

    def get_metrics(self):
        """Get worker metrics"""
        with self._lock:
            return self._metrics.copy()

    def queue_task(self, task_type, **kwargs):
        """Queue a new task with validation"""
        if task_type not in ['grow', 'cull']:
            raise ValueError(f"Invalid task type: {task_type}")

        task = {'type': task_type, **kwargs}
        try:
            self.task_queue.put(task, timeout=1.0)
            return True
        except queue.Full:
            logger.warning("Task queue full, task dropped")
            return False

    def get_result(self, timeout=0.1):
        """Get a result with timeout"""
        try:
            return self.result_queue.get(timeout=timeout)
        except queue.Empty:
            return None

    @property
    def is_alive(self):
        """Check if worker is alive and healthy"""
        return (super().is_alive() and 
                not self.stop_event.is_set() and 
                self._error_count < self._max_retries)

class PyGNeuralSystem:
    def __init__(self, sensory_width: int, sensory_height: int, n_dynamic: int, 
                 workspace_size: Tuple[int, int] = (16, 16), device: str = 'cuda'):
        self.device = device
        self.sensory_width = sensory_width
        self.sensory_height = sensory_height
        self.n_sensory_target = sensory_width * sensory_height
        self.n_dynamic_target = n_dynamic
        self.workspace_size = workspace_size
        self.n_workspace_target = workspace_size[0] * workspace_size[1]
        
        # Initialize counters
        self.n_sensory = 0
        self.n_dynamic = 0
        self.n_workspace = 0
        self.n_total = 0
        
        # Initialize metrics
        self.node_births = 0
        self.node_deaths = 0
        self.conn_births = 0
        self.conn_deaths = 0
        self.total_node_births = 0
        self.total_node_deaths = 0
        self.total_conn_births = 0
        self.total_conn_deaths = 0
        self.step_counter = 0
        
        # Initialize queues
        self.death_queue = []
        self.birth_queue = []
        self.conn_growth_queue = []
        self.conn_candidate_queue = []
        
        # Initialize graph
        self._init_empty_graph()
        
        # Initialize thread pool for parallel processing
        self.thread_pool = ThreadPoolExecutor(max_workers=4)
        
        # Initialize logging
        self.logger = logging.getLogger(__name__)

        # Initialize CUDA module if using GPU
        self.cuda_module = CUDAModule() if device == 'cuda' and CUDAModule is not None else None

    def _init_empty_graph(self):
        """Initialize an empty graph with PyTorch Geometric"""
        # Create empty node features
        node_data = NodeData(
            energy=torch.zeros((0, 1), device=self.device),
            node_type=torch.zeros((0,), dtype=torch.int64, device=self.device),
            pos=torch.zeros((0, 2), device=self.device),
            velocity=torch.zeros((0, 2), device=self.device)
        )
        
        # Create empty edge features
        edge_data = EdgeData(
            weight=torch.zeros((0, 1), device=self.device),
            energy_transfer_capacity=torch.zeros((0, 1), device=self.device),
            conn_type=torch.zeros((0, 1), dtype=torch.int64, device=self.device),
            plastic_lr=torch.zeros((0, 1), device=self.device),
            gate_threshold=torch.zeros((0, 1), device=self.device),
            conn_subtype3=torch.zeros((0,), dtype=torch.int64, device=self.device)
        )
        
        # Create empty edge index
        edge_index = torch.zeros((2, 0), dtype=torch.long, device=self.device)
        
        # Create PyG Data object
        self.graph = Data(
            x=node_data.energy,
            edge_index=edge_index,
            edge_attr=edge_data.weight,
            node_type=node_data.node_type,
            pos=node_data.pos,
            velocity=node_data.velocity,
            dynamic_subtype=node_data.dynamic_subtype,
            dynamic_subtype2=node_data.dynamic_subtype2,
            dynamic_subtype3=node_data.dynamic_subtype3,
            dynamic_subtype4=node_data.dynamic_subtype4,
            max_connections=node_data.max_connections,
            phase_offset=node_data.phase_offset,
            parent=node_data.parent,
            edge_weight=edge_data.weight,
            edge_capacity=edge_data.energy_transfer_capacity,
            edge_type=edge_data.conn_type,
            edge_plastic_lr=edge_data.plastic_lr,
            edge_gate_threshold=edge_data.gate_threshold,
            edge_subtype3=edge_data.conn_subtype3
        )
        
        # Set grid dimensions
        self.grid_width = max(self.sensory_width, self.workspace_size[0])
        self.grid_height = max(self.sensory_height, self.workspace_size[1])

    def to(self, device: str):
        """Move the system to a different device"""
        self.device = device
        self.graph = self.graph.to(device)
        return self

    def summary(self):
        """Print a summary of the system state"""
        print(f"Nodes: {self.graph.num_nodes} (sensory: {self.n_sensory}, dynamic: {self.n_dynamic}, workspace: {self.n_workspace})")
        print(f"Edges: {self.graph.num_edges}")
        print(f"Node features: {[k for k in self.graph.keys if k.startswith('x_')]}")
        print(f"Edge features: {[k for k in self.graph.keys if k.startswith('edge_')]}")

    def update(self):
        """Main update method with error handling and validation"""
        try:
            # Process death queue
            if self.death_queue:
                self._remove_nodes(self.death_queue)
                self.death_queue.clear()

            # Process birth queue
            if self.birth_queue:
                for node_type, args in self.birth_queue:
                    self._add_nodes(1, node_type, **args)
                self.birth_queue.clear()

            # Update node energies
            self._update_energies()

            # Update metrics
            self.step_counter += 1

        except Exception as e:
            self.logger.error(f"Critical error in update: {str(e)}")
            self._attempt_recovery()

    def _attempt_recovery(self):
        """Attempt to recover from a critical error"""
        try:
            # Clear queues
            self.death_queue.clear()
            self.birth_queue.clear()
            self.conn_growth_queue.clear()
            self.conn_candidate_queue.clear()

            # Reinitialize graph if needed
            if not hasattr(self, 'graph') or self.graph is None:
                self._init_empty_graph()

            self.logger.info("System recovery completed")
        except Exception as e:
            self.logger.error(f"Recovery failed: {str(e)}")
            self.cleanup()

    def cleanup(self):
        """Clean up resources"""
        try:
            # Shutdown thread pool
            if hasattr(self, 'thread_pool'):
                self.thread_pool.shutdown(wait=True)

            # Clear queues
            self.death_queue.clear()
            self.birth_queue.clear()
            self.conn_growth_queue.clear()
            self.conn_candidate_queue.clear()

            # Clear graph data
            if hasattr(self, 'graph'):
                self.graph = None

            # Force garbage collection
            import gc
            gc.collect()

            # Clear CUDA cache if using GPU
            if self.device == 'cuda':
                torch.cuda.empty_cache()

            self.logger.info("System cleanup completed")
        except Exception as e:
            self.logger.error(f"Error during cleanup: {str(e)}")

    def __del__(self):
        """Destructor to ensure cleanup"""
        self.cleanup()

    def _add_nodes(self, n: int, node_type: int, parent_idx: Optional[int] = None):
        """Add nodes to the graph with proper attributes"""
        if n == 0:
            return

        # Create parent array
        parent_arr = torch.full((n,), -1, dtype=torch.int64, device=self.device)
        if parent_idx is not None:
            parent_arr[0] = parent_idx

        # For dynamic nodes, give 1% chance to spawn as highway
        if node_type == NODE_TYPE_DYNAMIC:
            # Randomly select 1% of nodes to be highway
            highway_mask = torch.rand(n, device=self.device) < 0.01
            n_highway = highway_mask.sum().item()
            n_dynamic = n - n_highway

            # Add dynamic nodes
            if n_dynamic > 0:
                dynamic_indices = torch.where(~highway_mask)[0]
                subtypes = torch.randint(0, 3, (n_dynamic,), device=self.device)
                subtypes2 = torch.randint(0, 3, (n_dynamic,), device=self.device)
                subtypes3 = torch.randint(0, 3, (n_dynamic,), device=self.device)
                subtype4 = torch.randint(0, 3, (n_dynamic,), device=self.device)
                pos_new = torch.rand(n_dynamic, 2, device=self.device) * 100
                max_conns = torch.randint(1, 6, (n_dynamic,), device=self.device)
                phase_offset = torch.rand(n_dynamic, device=self.device) * 2 * np.pi

                # Create new node data
                new_nodes = NodeData(
                    energy=torch.zeros(n_dynamic, 1, device=self.device),
                    node_type=torch.full((n_dynamic,), NODE_TYPE_DYNAMIC, dtype=torch.int64, device=self.device),
                    pos=pos_new,
                    velocity=torch.zeros(n_dynamic, 2, device=self.device),
                    dynamic_subtype=subtypes,
                    dynamic_subtype2=subtypes2,
                    dynamic_subtype3=subtypes3,
                    dynamic_subtype4=subtype4,
                    max_connections=max_conns,
                    phase_offset=phase_offset,
                    parent=parent_arr[dynamic_indices]
                )

                # Add nodes to graph
                self._add_nodes_to_graph(new_nodes)

            # Add highway nodes
            if n_highway > 0:
                highway_indices = torch.where(highway_mask)[0]
                pos_new = torch.rand(n_highway, 2, device=self.device) * 100

                # Create new node data
                new_nodes = NodeData(
                    energy=torch.zeros(n_highway, 1, device=self.device),
                    node_type=torch.full((n_highway,), NODE_TYPE_HIGHWAY, dtype=torch.int64, device=self.device),
                    pos=pos_new,
                    velocity=torch.zeros(n_highway, 2, device=self.device),
                    max_connections=torch.full((n_highway,), 1000, dtype=torch.int64, device=self.device),
                    parent=parent_arr[highway_indices]
                )

                # Add nodes to graph
                self._add_nodes_to_graph(new_nodes)
        else:
            # Original node type handling
            pos_new = torch.rand(n, 2, device=self.device) * 100

            # Create new node data
            new_nodes = NodeData(
                energy=torch.zeros(n, 1, device=self.device),
                node_type=torch.full((n,), node_type, dtype=torch.int64, device=self.device),
                pos=pos_new,
                velocity=torch.zeros(n, 2, device=self.device),
                parent=parent_arr
            )

            # Add nodes to graph
            self._add_nodes_to_graph(new_nodes)

    def _add_nodes_to_graph(self, node_data: NodeData):
        """Helper method to add nodes to the graph"""
        # Get current number of nodes
        n_current = self.graph.num_nodes
        n_new = len(node_data.energy)

        # Create new graph with combined data
        new_graph = Data(
            x=torch.cat([self.graph.x, node_data.energy], dim=0),
            edge_index=self.graph.edge_index,
            edge_attr=self.graph.edge_attr,
            node_type=torch.cat([self.graph.node_type, node_data.node_type], dim=0),
            pos=torch.cat([self.graph.pos, node_data.pos], dim=0),
            velocity=torch.cat([self.graph.velocity, node_data.velocity], dim=0)
        )

        # Add optional attributes if they exist
        if node_data.dynamic_subtype is not None:
            new_graph.dynamic_subtype = torch.cat([
                self.graph.dynamic_subtype if hasattr(self.graph, 'dynamic_subtype') and self.graph.dynamic_subtype is not None else 
                torch.full((n_current,), -1, dtype=torch.int64, device=self.device),
                node_data.dynamic_subtype
            ], dim=0)

        if node_data.dynamic_subtype2 is not None:
            new_graph.dynamic_subtype2 = torch.cat([
                self.graph.dynamic_subtype2 if hasattr(self.graph, 'dynamic_subtype2') and self.graph.dynamic_subtype2 is not None else 
                torch.full((n_current,), -1, dtype=torch.int64, device=self.device),
                node_data.dynamic_subtype2
            ], dim=0)

        if node_data.dynamic_subtype3 is not None:
            new_graph.dynamic_subtype3 = torch.cat([
                self.graph.dynamic_subtype3 if hasattr(self.graph, 'dynamic_subtype3') and self.graph.dynamic_subtype3 is not None else 
                torch.full((n_current,), -1, dtype=torch.int64, device=self.device),
                node_data.dynamic_subtype3
            ], dim=0)

        if node_data.dynamic_subtype4 is not None:
            new_graph.dynamic_subtype4 = torch.cat([
                self.graph.dynamic_subtype4 if hasattr(self.graph, 'dynamic_subtype4') and self.graph.dynamic_subtype4 is not None else 
                torch.full((n_current,), -1, dtype=torch.int64, device=self.device),
                node_data.dynamic_subtype4
            ], dim=0)

        if node_data.max_connections is not None:
            new_graph.max_connections = torch.cat([
                self.graph.max_connections if hasattr(self.graph, 'max_connections') and self.graph.max_connections is not None else 
                torch.full((n_current,), 5, dtype=torch.int64, device=self.device),
                node_data.max_connections
            ], dim=0)

        if node_data.phase_offset is not None:
            new_graph.phase_offset = torch.cat([
                self.graph.phase_offset if hasattr(self.graph, 'phase_offset') and self.graph.phase_offset is not None else 
                torch.zeros(n_current, device=self.device),
                node_data.phase_offset
            ], dim=0)

        if node_data.parent is not None:
            new_graph.parent = torch.cat([
                self.graph.parent if hasattr(self.graph, 'parent') and self.graph.parent is not None else 
                torch.full((n_current,), -1, dtype=torch.int64, device=self.device),
                node_data.parent
            ], dim=0)

        # Update the graph
        self.graph = new_graph

        # Update counters
        if node_data.node_type[0] == NODE_TYPE_SENSORY:
            self.n_sensory += n_new
        elif node_data.node_type[0] == NODE_TYPE_DYNAMIC:
            self.n_dynamic += n_new
        elif node_data.node_type[0] == NODE_TYPE_WORKSPACE:
            self.n_workspace += n_new
        self.n_total += n_new

    def _remove_nodes(self, node_indices: List[int]):
        """Remove nodes from the graph"""
        if not node_indices:
            return

        # Convert to tensor
        node_indices = torch.tensor(node_indices, dtype=torch.long, device=self.device)
        
        # Create mask for nodes to keep
        mask = torch.ones(self.graph.num_nodes, dtype=torch.bool, device=self.device)
        mask[node_indices] = False

        # Create new graph with remaining nodes
        new_graph = Data(
            x=self.graph.x[mask],
            edge_index=self.graph.edge_index,
            edge_attr=self.graph.edge_attr,
            node_type=self.graph.node_type[mask],
            pos=self.graph.pos[mask],
            velocity=self.graph.velocity[mask]
        )

        # Update optional attributes
        for key in ['dynamic_subtype', 'dynamic_subtype2', 'dynamic_subtype3', 
                   'dynamic_subtype4', 'max_connections', 'phase_offset', 'parent']:
            if hasattr(self.graph, key) and getattr(self.graph, key) is not None:
                setattr(new_graph, key, getattr(self.graph, key)[mask])

        # Update edge indices
        if self.graph.num_edges > 0:
            # Remove edges connected to removed nodes
            edge_mask = torch.ones(self.graph.num_edges, dtype=torch.bool, device=self.device)
            for idx in node_indices:
                edge_mask &= (self.graph.edge_index[0] != idx) & (self.graph.edge_index[1] != idx)
            
            new_graph.edge_index = self.graph.edge_index[:, edge_mask]
            new_graph.edge_attr = self.graph.edge_attr[edge_mask]

            # Update edge indices to account for removed nodes
            for idx in sorted(node_indices, reverse=True):
                new_graph.edge_index[self.graph.edge_index > idx] -= 1

        # Update the graph
        self.graph = new_graph

        # Update counters
        removed_types = self.graph.node_type[node_indices]
        self.n_sensory -= (removed_types == NODE_TYPE_SENSORY).sum().item()
        self.n_dynamic -= (removed_types == NODE_TYPE_DYNAMIC).sum().item()
        self.n_workspace -= (removed_types == NODE_TYPE_WORKSPACE).sum().item()
        self.n_total -= len(node_indices)

    def _update_energies(self):
        """Update energy levels for all nodes using CUDA kernels"""
        if self.graph.num_nodes == 0:
            return

        # Get node types and energies
        node_types = self.graph.node_type
        energies = self.graph.x

        # Process dynamic nodes
        dynamic_mask = (node_types == NODE_TYPE_DYNAMIC)
        if dynamic_mask.any():
            # Calculate energy decay for dynamic nodes
            out_deg = torch_geometric.utils.degree(self.graph.edge_index[0], num_nodes=self.graph.num_nodes)
            decay = out_deg[dynamic_mask] * CONNECTION_MAINTENANCE_COST
            energies[dynamic_mask] -= decay.unsqueeze(1)

            # Get dynamic node subtypes (with safe access)
            if hasattr(self.graph, 'dynamic_subtype') and self.graph.dynamic_subtype is not None:
                dynamic_subtypes = self.graph.dynamic_subtype[dynamic_mask]
                
                # Apply subtype-specific energy modifications
                # Transmitter: Boost outgoing energy
                transmitter_mask = (dynamic_subtypes == SUBTYPE_TRANSMITTER)
                if transmitter_mask.any():
                    energies[dynamic_mask][transmitter_mask] *= 1.1

                # Resonator: More efficient energy reception
                resonator_mask = (dynamic_subtypes == SUBTYPE_RESONATOR)
                if resonator_mask.any():
                    energies[dynamic_mask][resonator_mask] *= 1.05

                # Dampener: Reduce decay
                dampener_mask = (dynamic_subtypes == SUBTYPE_DAMPENER)
                if dampener_mask.any():
                    decay[dampener_mask] *= 0.8

        # Process energy transfers
        if self.graph.num_edges > 0:
            if self.cuda_module is not None:
                # Use CUDA kernel for energy transfer
                energies = self.cuda_module.energy_transfer(
                    energies,
                    self.graph.edge_index,
                    self.graph.edge_attr.squeeze(),
                    getattr(self.graph, 'edge_capacity', torch.ones_like(self.graph.edge_attr.squeeze())),
                    getattr(self.graph, 'edge_type', torch.zeros_like(self.graph.edge_attr.squeeze(), dtype=torch.int64)),
                    getattr(self.graph, 'edge_gate_threshold', torch.zeros_like(self.graph.edge_attr.squeeze())),
                    getattr(self.graph, 'edge_subtype3', torch.zeros_like(self.graph.edge_attr.squeeze(), dtype=torch.int64)),
                    getattr(self.graph, 'parent', torch.full((self.graph.num_nodes,), -1, dtype=torch.int64, device=self.device)),
                    TRANSMISSION_LOSS
                )

                # Update plastic connections using CUDA kernel
                edge_type = getattr(self.graph, 'edge_type', torch.zeros(self.graph.num_edges, dtype=torch.int64, device=self.device))
                plastic_mask = (edge_type == CONN_TYPE_PLASTIC)
                if plastic_mask.any():
                    edge_plastic_lr = getattr(self.graph, 'edge_plastic_lr', torch.zeros(self.graph.num_edges, device=self.device))
                    self.graph.edge_attr[plastic_mask] = self.cuda_module.plastic_update(
                        self.graph.edge_attr[plastic_mask],
                        energies[self.graph.edge_index[0][plastic_mask]],
                        energies[self.graph.edge_index[1][plastic_mask]],
                        edge_plastic_lr[plastic_mask],
                        self.graph.edge_index[:, plastic_mask]
                    )
            else:
                # Fallback to CPU implementation
                self._cpu_energy_transfer(energies)

        # Process highway nodes
        highway_mask = (node_types == NODE_TYPE_HIGHWAY)
        if highway_mask.any():
            if self.cuda_module is not None:
                # Use CUDA kernel for highway node balancing
                energies = self.cuda_module.highway_balance(
                    energies,
                    node_types,
                    self.graph.edge_index,
                    self.graph.edge_attr.squeeze(),
                    0.1  # balance_rate
                )
            else:
                # Fallback to CPU implementation
                self._cpu_highway_balance(energies, highway_mask, dynamic_mask)

        # Apply energy caps and death threshold
        energies.clamp_(NODE_DEATH_THRESHOLD, NODE_ENERGY_CAP)

        # Kill nodes that are below death threshold
        dead_nodes = (energies <= NODE_DEATH_THRESHOLD).squeeze()
        if dead_nodes.any():
            self._remove_nodes(dead_nodes.nonzero(as_tuple=True)[0].tolist())

        # Emergency shutdown if energy distribution is too extreme
        if energies.std() > NODE_ENERGY_CAP * 0.5:  # If standard deviation exceeds 50% of cap
            logger.warning("Emergency shutdown triggered due to extreme energy distribution")
            self.suspended = True

    def _cpu_energy_transfer(self, energies):
        """CPU fallback for energy transfer"""
        src, dst = self.graph.edge_index
        weights = self.graph.edge_attr.squeeze()
        
        # Get edge attributes with safe defaults
        conn_types = getattr(self.graph, 'edge_type', torch.zeros_like(weights, dtype=torch.int64))
        energy_caps = getattr(self.graph, 'edge_capacity', torch.ones_like(weights))
        gate_thresholds = getattr(self.graph, 'edge_gate_threshold', torch.zeros_like(weights))
        conn_subtype3 = getattr(self.graph, 'edge_subtype3', torch.zeros_like(weights, dtype=torch.int64))
        parent = getattr(self.graph, 'parent', torch.full((self.graph.num_nodes,), -1, dtype=torch.int64, device=self.device))

        # Calculate allowed connections
        is_parent = (parent[dst] == src)
        allowed = (
            (conn_subtype3 == CONN_SUBTYPE3_FREE_FLOW) |
            ((conn_subtype3 == CONN_SUBTYPE3_ONE_WAY_OUT) & is_parent) |
            ((conn_subtype3 == CONN_SUBTYPE3_ONE_WAY_IN) & (~is_parent))
        )

        # Process different connection types
        # Excitatory connections
        excitatory_mask = (conn_types == CONN_TYPE_EXCITATORY) & allowed
        if excitatory_mask.any():
            src_exc = src[excitatory_mask]
            dst_exc = dst[excitatory_mask]
            weights_exc = weights[excitatory_mask]
            caps_exc = energy_caps[excitatory_mask]
            transfer = energies[src_exc] * weights_exc.unsqueeze(1) * caps_exc.unsqueeze(1)
            energies.index_add_(0, dst_exc, transfer)
            energies.index_add_(0, src_exc, -transfer)

        # Similar processing for other connection types...
        # (Implementation continues with inhibitory, gated, and plastic connections)

    def _cpu_highway_balance(self, energies, highway_mask, dynamic_mask):
        """CPU fallback for highway node balancing"""
        # Normalize highway node energies
        highway_energies = energies[highway_mask]
        avg_highway_energy = highway_energies.mean()
        energy_diff = avg_highway_energy - highway_energies
        transfer_amount = energy_diff * 0.1
        energies[highway_mask] += transfer_amount

        # Highway nodes pull energy from dynamic nodes
        if dynamic_mask.any():
            # Find highway-dynamic connections
            src, dst = self.graph.edge_index
            node_types = self.graph.node_type
            highway_src = (node_types[src] == NODE_TYPE_HIGHWAY)
            dynamic_dst = (node_types[dst] == NODE_TYPE_DYNAMIC)
            pull_mask = highway_src & dynamic_dst

            if pull_mask.any():
                src_pull = src[pull_mask]
                dst_pull = dst[pull_mask]
                src_energy = energies[src_pull]
                dst_energy = energies[dst_pull]
                
                # Calculate pull amount
                dst_energy_needed = NODE_ENERGY_CAP - dst_energy
                pull_amount = torch.minimum(
                    src_energy * 0.2,
                    dst_energy_needed
                )
                
                # Apply pull
                energies.index_add_(0, src_pull, -pull_amount)
                energies.index_add_(0, dst_pull, pull_amount)

    def get_metrics(self):
        """Get system metrics"""
        node_types = self.graph.node_type
        n_sensory = int((node_types == NODE_TYPE_SENSORY).sum().cpu().item())
        n_dynamic = int((node_types == NODE_TYPE_DYNAMIC).sum().cpu().item())
        n_workspace = int((node_types == NODE_TYPE_WORKSPACE).sum().cpu().item())
        
        dynamic_energies = self.graph.x[node_types == NODE_TYPE_DYNAMIC].cpu().numpy().flatten()
        avg_dynamic_energy = float(dynamic_energies.mean()) if dynamic_energies.size > 0 else 0.0
        total_energy = float(self.graph.x.sum().cpu().item())
        
        return {
            'total_energy': total_energy,
            'avg_dynamic_energy': avg_dynamic_energy,
            'sensory_node_count': n_sensory,
            'dynamic_node_count': n_dynamic,
            'workspace_node_count': n_workspace,
            'node_births': self.node_births,
            'node_deaths': self.node_deaths,
            'conn_births': self.conn_births,
            'conn_deaths': self.conn_deaths,
            'total_node_births': self.total_node_births,
            'total_node_deaths': self.total_node_deaths,
            'total_conn_births': self.total_conn_births,
            'total_conn_deaths': self.total_conn_deaths,
            'connection_count': self.graph.num_edges,
        }

    # Additional methods for connection management, validation, etc.
    # (Truncated for brevity, but would include all remaining methods from original file)

    def _prepare_connection_growth_batch(self, batch_size):
        """Prepare a batch of connection growth candidates"""
        # Implementation would be here
        return []

    def _prepare_cull_batch(self, batch_size):
        """Prepare a batch of connections to cull"""
        # Implementation would be here
        return []

    def start_connection_worker(self, batch_size=25):
        """Start the connection worker thread"""
        self.connection_worker = ConnectionWorker(self, batch_size=batch_size)
        self.connection_worker.start()

    def stop_connection_worker(self):
        """Stop the connection worker thread"""
        if hasattr(self, 'connection_worker'):
            self.connection_worker.stop()

    def queue_connection_growth(self):
        """Queue a connection growth task"""
        if hasattr(self, 'connection_worker'):
            self.connection_worker.queue_task('grow')

    def queue_cull(self):
        """Queue a connection culling task"""
        if hasattr(self, 'connection_worker'):
            self.connection_worker.queue_task('cull')

    def apply_connection_worker_results(self):
        """Apply results from the connection worker"""
        if not hasattr(self, 'connection_worker'):
            return
        # Implementation would be here