...
def run_main_loop(graph, steps=1000, step_delay=0.01):
    ...
    for step in range(steps):
        ...
        # NEW: Apply homeostatic regulation based on configuration
        from config_manager import get_homeostasis_config
        homeostasis_config = get_homeostasis_config()
        regulation_interval = homeostasis_config.get('regulation_interval', 100)
        
        if step % regulation_interval == 0:
        ...
        # NEW: Calculate network metrics every 50 steps
        if step % 50 == 0:
            log_step("Calculate network metrics", step=step)
            
            # Initialize network metrics engine if not already done
            if not hasattr(graph, 'network_metrics'):
                from config_manager import get_network_metrics_config
                config = get_network_metrics_config()
                graph.network_metrics = NetworkMetrics(calculation_interval=config.get('calculation_interval', 50))
            
            # Calculate comprehensive network metrics
            metrics = graph.network_metrics.calculate_comprehensive_metrics(graph)
            
            # Log key metrics for monitoring
            logging.info(f"[NETWORK_METRICS] Step {step}: Criticality={metrics['criticality']:.3f}, "
            ...
            health_score = graph.network_metrics.get_network_health_score()
            if health_score['score'] < 60.0:
                logging.warning(f"[NETWORK_METRICS] Low network health score: {health_score['score']:.1f} - {health_score['status']}")
                for rec in health_score['recommendations']:
                    logging.warning(f"[NETWORK_METRICS] Recommendation: {rec}")
            
            logging.info(f"[MAIN_LOOP] Network metrics calculated", extra={"step": step})
