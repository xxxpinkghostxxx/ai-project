...
class NodeIDManager:
    ...
    def __init__(self):
        # Use reader-writer lock for better concurrency
        self._rw_lock = get_id_manager_lock()
        self._next_id = 1
        ...
        log_step("NodeIDManager initialized with atomic transaction support and reader-writer locks")
    @contextmanager
    ...
    def _execute_generate_id(self, operation: Dict[str, Any]):
        """Execute ID generation within transaction (write operation)."""
    ...
    def generate_unique_id(self, node_type: str = "unknown", metadata: Optional[Dict[str, Any]] = None) -> int:
        """Generate a unique ID using atomic transaction."""
    ...
    def _find_next_available_id(self) -> Optional[int]:
        """Find next available ID (read operation)."""
    ...
    def register_node_index(self, node_id: int, index: int) -> bool:
        """Register node index using atomic transaction."""
        try:
            with self.transaction() as txn:
    ...
    def recycle_node_id(self, node_id: int) -> bool:
        """Recycle node ID using atomic transaction."""
        try:
            with self.transaction() as txn:
    ...
    def reset(self):
        """Reset the ID manager (write operation)."""
        with self._rw_lock.write_lock():
            self._next_id = 1
